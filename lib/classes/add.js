	...
	async run(msg, args) {
		...
		if(args.options) {
			if(this.multipleOptions) {
				...
				let res = this.pushToTags(msg, args, list);
				...
				if (this.list.hasOwnProperty(args.item)) {
					return alerts.sendError(msg, `\`${args.item}\` already exists. Please use another value.`);
				}

				list[args.item] = args.options;

				replyMsg = commonTags.commaLists`\`${args.item}\` was added to \`${args.options}\``;
			}
		} else {
			list.push(args.item);
			replyMsg = `\`${args.item}\` was added.`;
		}
